    ...
    try {
        ...
        for (const dias of periodos) {
            ...
            const clientesInativos = ultimasCompras
                .filter(compra => compra._max.dataEmissao &&
            ...
            const valorTotalPerdido = Object.values(ultimasComprasPorCliente)
                .reduce((sum, compra) => sum + Number(compra.valorTotal), 0);
            resultados.push({
                periodo: `>${dias} dias`,
...
const getClientesNovosRecorrentes = async (req, res) => {
    try {
        ...
        const whereClausePrimeiraCompra = {
            clienteId: { not: null },
        };
        if (filialId) {
        ...
        const mapaPrimeirasCompras = new Map();
        ...
        for (const grupo of clientesParaAnalise) {
            ...
            if (comprasCliente.length < 2) {
                ...
                resultados.push({
                    ...
                    primeiraCompra: ((_a = comprasCliente[0]) === null || _a === void 0 ? void 0 : _a.dataEmissao) || null,
                    ultimaCompra: ((_b = comprasCliente[0]) === null || _b === void 0 ? void 0 : _b.dataEmissao) || null,
                    tempoVidaDias: 0,
                    intervaloMedioDias: null,
                    valorTotalGasto: Number(((_c = comprasCliente[0]) === null || _c === void 0 ? void 0 : _c.valorTotal) || 0),
                });
                continue;
            }
            ...
            for (let i = 1; i < comprasCliente.length; i++) {
                const intervalo = Math.floor((comprasCliente[i].dataEmissao.getTime() - comprasCliente[i - 1].dataEmissao.getTime()) /
                    (1000 * 60 * 60 * 24));
...
const getConcentracaoReceita = async (req, res) => {
    try {
        ...
        const receitaTotal = receitaPorCliente.reduce((sum, cliente) => sum + Number(cliente._sum.valorTotal || 0), 0);
...
const getCohortAnalysis = async (req, res) => {
    try {
        ...
        const mesesAnaliseInt = parseInt(mesesAnalise);
        const whereClause = {
            clienteId: { not: null },
        };
        if (filialId) {
        ...
        const primeirasCompras = new Map();
...
const getPosVendaPercentual = async (req, res) => {
    try {
        ...
        // Definir período de análise
        const inicio = dataInicio ? new Date(dataInicio) : new Date(new Date().setFullYear(new Date().getFullYear() - 1));
        ...
        const comprasComMaquinas = comprasMaquinas.filter(compra => compra.itens.some(item => item.produto.tipo === 'Máquina'));
        const periodos = [30, 60, 90];
        const resultados = [];
        for (const dias of periodos) {
            const clientesComMaquina = new Set();
            const clientesComPosVenda = new Set();
            ...
            for (const compra of comprasComMaquinas) {
                ...
                const temPosVenda = comprasPosVenda.some(compraPV => compraPV.itens.some(item => item.produto.tipo === 'Serviço' || item.produto.tipo === 'Peça'));
                if (temPosVenda) {
