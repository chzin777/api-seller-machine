...
const recalcularAssociacoes = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        for (const item of itens) {
            ...
            const clienteId = ((_b = (_a = item.notaFiscal) === null || _a === void 0 ? void 0 : _a.cliente) === null || _b === void 0 ? void 0 : _b.id) || `nota_${(_c = item.notaFiscal) === null || _c === void 0 ? void 0 : _c.id}`;
            if (!vendas[clienteId])
                vendas[clienteId] = [];
            vendas[clienteId].push({
                produtoId: item.produtoId,
                clienteId: (_e = (_d = item.notaFiscal) === null || _d === void 0 ? void 0 : _d.cliente) === null || _e === void 0 ? void 0 : _e.id,
                notaFiscalId: (_f = item.notaFiscal) === null || _f === void 0 ? void 0 : _f.id
            });
        }
        ...
        for (const [clienteId, itensVenda] of Object.entries(vendas)) {
            ...
            for (const a of produtos) {
                if (!vendasPorProduto[a])
                    vendasPorProduto[a] = new Set();
        ...
        for (const { a, b, suporte, clientes } of Object.values(pares)) {
            if (a === b)
                continue;
            const vendasA = ((_g = vendasPorProduto[a]) === null || _g === void 0 ? void 0 : _g.size) || 0;
            const vendasB = ((_h = vendasPorProduto[b]) === null || _h === void 0 ? void 0 : _h.size) || 0;
            ...
            associacoesBulk.push({
                ...
                confianca,
                lift,
                a_nome: ((_j = produtosMap[a]) === null || _j === void 0 ? void 0 : _j.descricao) || '',
                b_nome: ((_k = produtosMap[b]) === null || _k === void 0 ? void 0 : _k.descricao) || '',
                a_tipo: ((_l = produtosMap[a]) === null || _l === void 0 ? void 0 : _l.tipo) || '',
                b_tipo: ((_m = produtosMap[b]) === null || _m === void 0 ? void 0 : _m.tipo) || '',
...
const getAssociacoes = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (process.env.NODE_ENV !== 'production') {
            console.log('Prisma keys:', Object.keys(index_1.prisma));
        }
        // Filtros opcionais
        const { produto_a_id, produto_b_id, tipo, periodo_inicio, periodo_fim, page = 1, pageSize = 20, } = req.query;
        const where = {};
        if (produto_a_id)
            where.produto_a_id = Number(produto_a_id);
        if (produto_b_id)
            where.produto_b_id = Number(produto_b_id);
        if (periodo_inicio || periodo_fim) {
            where.atualizado_em = {};
            if (periodo_inicio)
                where.atualizado_em.gte = new Date(periodo_inicio);
            if (periodo_fim)
                where.atualizado_em.lte = new Date(periodo_fim);
        }
        ...
        const result = associacoes.map((a) => ({
