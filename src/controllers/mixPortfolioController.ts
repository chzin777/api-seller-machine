...
export const getMixPorTipo = async (req: Request, res: Response) => {
  try {
...
export const getPrecoRealizadoVsReferencia = async (req: Request, res: Response) => {
  try {
    ...
    const tiposPermitidos = tipos ? (tipos as string).split(',') : ['Peça', 'Serviço'];
...
export const getBundleRate = async (req: Request, res: Response) => {
  try {
    const { filialId, dataInicio, dataFim } = req.query;
...
export const getProdutosSemGiro = async (req: Request, res: Response) => {
  try {
    ...
    if (tipo) {
      whereClauseProdutos.tipo = tipo as string;
    }
