...
import { clusteringModel } from '../models/clusteringModel';
import DataPreprocessor from '../models/dataPreprocessor';
...
export const getRecommendations = async (req: Request, res: Response) => {
  try {
    ...
    const produtosComprados = new Set<number>();
    let valorTotalGasto = 0;
    let tiposPreferidos: { [key: string]: number } = {};
    ...
    const recomendacoes = associacoes
      ...
      .map(assoc => {
        ...
        const scoreRecomendacao = (
          Number(assoc.lift) * 0.4 +
...
export const getChurnPrediction = async (req: Request, res: Response) => {
  try {
    ...
    const clientesMetricas = new Map<number, any>();
    ...
    const clientesComChurn = Array.from(clientesMetricas.values()).map(metrics => {
      ...
      if (diasSemComprar > 120) churnScore += 40;
      else if (diasSemComprar > 90) churnScore += 30;
      else if (diasSemComprar > 60) churnScore += 20;
      else if (diasSemComprar > 30) churnScore += 10;
      
      // Fator frequência (30% do score)
      if (metrics.totalCompras < 2) churnScore += 30;
      else if (metrics.totalCompras < 5) churnScore += 20;
      else if (metrics.totalCompras < 10) churnScore += 10;
      
      // Fator valor (30% do score)
      if (ticketMedio < 1000) churnScore += 30;
      else if (ticketMedio < 5000) churnScore += 20;
      else if (ticketMedio < 10000) churnScore += 10;
      
      let risco = 'Baixo';
      if (churnScore >= 70) risco = 'Alto';
      else if (churnScore >= 40) risco = 'Médio';
...
export const getSalesPrediction = async (req: Request, res: Response) => {
  try {
    ...
    const vendasPorMes = new Map<string, { valor: number, quantidade: number }>();
    ...
    // Gerar predições
    const predicoes = [];
    const mesesPred = parseInt(mesesPredicao as string);
    ...
    res.json({
      historicoAnalisado: {
        ...
        periodoInicio: serieHistorica[0]?.mes,
        periodoFim: serieHistorica[serieHistorica.length - 1]?.mes,
...
function generateChurnRecommendations(churnScore: number, diasSemComprar: number, totalCompras: number): string[] {
  const recomendacoes = [];
 ...
 */
export const getRFVOptimization = async (req: Request, res: Response) => {
  try {
    ...
    const clientesRFV = new Map<number, any>();
...
function optimizeRFVWeights(dados: any[]): { recencia: number, frequencia: number, valor: number } {
...
function calculateCV(valores: number[]): number {
...
function applyOptimizedSegmentation(dados: any[], pesos: any) {
  ...
  const segmentos = dados.map((cliente, i) => {
    const scoreRFV = (
      recenciasNorm[i] * pesos.recencia +
      frequenciasNorm[i] * pesos.frequencia +
      valoresNorm[i] * pesos.valor
    );
    
    let segmento = 'Bronze';
    if (scoreRFV >= 0.8) segmento = 'Diamante';
    else if (scoreRFV >= 0.6) segmento = 'Ouro';
    else if (scoreRFV >= 0.4) segmento = 'Prata';
  ...
  const grupos = segmentos.reduce((acc: any, cliente) => {
    if (!acc[cliente.segmento]) acc[cliente.segmento] = [];
...
function normalizeArray(valores: number[], inverso = false): number[] {
  const min = Math.min(...valores);
  const max = Math.max(...valores);
  const range = max - min;
  
  if (range === 0) return valores.map(() => 0.5);
 ...
 */
export const trainChurnModel = async (req: Request, res: Response) => {
  try {
    const { filialId } = req.query;
    const filial = filialId ? parseInt(filialId as string) : undefined;

    console.log('Iniciando treinamento do modelo de churn com TensorFlow.js...');
    const result = await churnModel.trainModel(filial);
 ...
 */
export const predictChurnML = async (req: Request, res: Response) => {
  try {
    const { clienteId } = req.params;
    ...
    const prediction = await churnModel.predictChurn(id);
...
export const batchPredictChurn = async (req: Request, res: Response) => {
  try {
    ...
    const predictions = await churnModel.batchPredict(filial, maxLimit);
 ...
 */
export const trainRecommendationModel = async (req: Request, res: Response) => {
  try {
    const { filialId } = req.query;
    const filial = filialId ? parseInt(filialId as string) : undefined;

    console.log('Iniciando treinamento do modelo de recomendação...');
    const result = await recommendationModel.trainModel(filial);
 ...
 */
export const getMLRecommendations = async (req: Request, res: Response) => {
  try {
    const { clienteId } = req.params;
    const { topK } = req.query;
    const id = parseInt(clienteId);
    const limit = topK ? parseInt(topK as string) : 10;
    ...
    try {
      const recommendations = await recommendationModel.recommendProducts(id, limit);
      ...
      const popularProducts = await recommendationModel.getPopularProducts(undefined, limit);
...
export const getSimilarProducts = async (req: Request, res: Response) => {
  try {
    ...
    const limit = topK ? parseInt(topK as string) : 5;

    if (isNaN(id)) {
      return res.status(400).json({ error: 'ID do produto inválido' });
    }

    const similarProducts = await recommendationModel.getSimilarProducts(id, limit);
 ...
 */
export const trainClusteringModel = async (req: Request, res: Response) => {
  try {
    const { filialId } = req.query;
    const filial = filialId ? parseInt(filialId as string) : undefined;

    console.log('Iniciando treinamento do modelo de clustering...');
    const result = await clusteringModel.trainModel(filial);

    res.json({
      success: true,
      message: 'Modelo de clustering treinado com sucesso',
      clusterAssignments: result.clusterAssignments?.slice(0, 10), // Mostrar apenas alguns exemplos
      timestamp: new Date().toISOString()
    });
  } catch (error) {
 ...
 */
export const getCustomerClusters = async (req: Request, res: Response) => {
  try {
    const { filialId } = req.query;
    const filial = filialId ? parseInt(filialId as string) : undefined;

    const clusterResult = await clusteringModel.clusterCustomers(filial);
    ...
    res.json({
      ...
      insights: {
        largestCluster: clusterStats.reduce((max, cluster) => 
          cluster.count > max.count ? cluster : max
        ),
        smallestCluster: clusterStats.reduce((min, cluster) => 
 ...
 */
export const getDataAnalysis = async (req: Request, res: Response) => {
  try {
    const { filialId } = req.query;
    const filial = filialId ? parseInt(filialId as string) : undefined;
    ...
    const stats = DataPreprocessor.calculateStatistics(features);
    
    // Calcular matriz de correlação
    const correlationMatrix = DataPreprocessor.calculateCorrelationMatrix(features);
    
    // Detectar outliers
    const { removedIndices } = DataPreprocessor.removeOutliers(features);
    
    // Normalizar dados para visualização
    const { normalizedData } = DataPreprocessor.normalizeMinMax(features);
    ...
    res.json({
      ...
      correlationMatrix: correlationMatrix.map(row => 
        row.map(val => typeof val === 'number' && !isNaN(val) ? parseFloat(val.toFixed(3)) : 0)
      ),
      insights: {
...
export const getCustomerInsights = async (req: Request, res: Response) => {
  try {
      ...
      nf.itens?.forEach(item => {
        ...
        if (produtosComprados.has(produtoId)) {
          ...
          produtosComprados.set(produtoId, {
            produtoId,
            nome: item.produto?.descricao || 'Produto não encontrado',
            quantidade,
    ...
    let churnScore = 0;
    if (diasSemComprar > 90) churnScore += 0.3;
    if (diasSemComprar > 180) churnScore += 0.3;
    if (frequenciaMensal < 1) churnScore += 0.2;
    if (ticketMedio < 100) churnScore += 0.2;
    ...
    res.json({
      ...
      insights: {
        ...
        produtoFavorito: topProdutos[0]?.nome || 'Nenhum',
...
function findHighestCorrelation(matrix: number[][], featureNames: string[]) {
  let maxCorr = 0;
